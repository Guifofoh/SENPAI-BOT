...
* Há um ódio enorme do lado de Deus que Você diz qualquer coisa que não faz."
* (QS ash-Shaff: 2-3).
*/
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            ...
            error: {
                St: '[❗] Envie a imagem com a legenda *! Adesivo * ou a tag da imagem que foi enviada',
                ...
                Ki: '[❗] Bot não pode emitir administrador de grupo!',
                Ad: '[❗] Incapaz de adicionar alvo, talvez porque é privado',
                Iv: '[❗] O link que você enviou é inválido!'
            }
        }
        const apiKey = 'API-KEY' // Chave API você pode obtê-lo em https://mhankbarbars.herokuapp.com/api
        ...
        case '!sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    client.reply(from, '[WAIT] Sendo processado⏳ Por favor, espere ± 1 min!', id)
                    ...
                    client.reply(from, '[❗] Envie um vídeo com uma legenda *!stickerGif* max 10 seg!', id)
                )
            }
            break
	    case '!stickernobg':
            ...
            if (args.length === 2) {
                ...
                if (cek === true)  {
                    try {
                        ...
                        client.reply(from, '[❗] Algo deu errado, talvez uma sala de código nuclearh', id)
                    }
                } else {
        ...
        case '!listblock':
            ...
            break
        case '!jadwalshalat':
            if (args.length === 1) return client.reply(from, '[❗] Enviar comando *! Momentos de oração [área]*\ncontoh : *!jadwalShalat Tangerang*\nUntuk list daerah kirim perintah *!listDaerah*')
